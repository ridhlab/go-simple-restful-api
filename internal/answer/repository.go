...
func (r *AnswerRepository) GetAnswerById(id int) (*models.Answer, error) {
	query := "SELECT id, question_id, user_id, content, created_at FROM answers WHERE id = ?"
...
func (r *AnswerRepository) GetAnswerByUserId(id int) ([]*models.Answer, error) {
	query := "SELECT id, question_id, user_id, content, created_at FROM answers WHERE user_id = ?"
...
func (r *AnswerRepository) UpdateAnswer(answer *models.Answer) error {
	query := "UPDATE answers SET question_id = ?, user_id = ?, content = ? WHERE id = ?"
...
func (r *AnswerRepository) GetAnswerByQuestionId(id int) ([]*models.Answer, error) {
	query := "SELECT id, question_id, user_id, content, created_at FROM answers WHERE question_id = ?"
	rows, err := r.db.Query(query, id)
	if err != nil {
		return nil, err
	}
